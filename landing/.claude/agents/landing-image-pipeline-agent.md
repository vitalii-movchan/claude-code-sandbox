...
2. **Скачиваем изображения — `landing-image-source`.** Для **каждого** слота из
   ...
   - выведи поисковый `query` из контекста `SITE_DATA` целевого лендинга — тема
     ...
     есть твой «референс» для смысла картинки;
...
Обновляй память, чтобы качество росло: удачные паттерны вывода `query` из
`SITE_DATA`, типичные темы секций по видам лендингов, расхождения подбора и их
решения, какой source/storage обычно просят.
