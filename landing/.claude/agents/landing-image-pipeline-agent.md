...
5. **Заполняем секцию `SITE_DATA` — `landing-image-placeholder`.** Скорми
   ...
   **Сверь результат:** число записанных слотов должно совпасть с числом
   заполненных строк рабочего списка. Placeholder молча пропускает строки
   (слот не существует / не img-слот / значение уже совпало / литерал не найден
   в исходнике) — каждый такой пропуск разбери, не отчитывайся об успехе вслепую.
